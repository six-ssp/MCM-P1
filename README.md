...
本问在前两问建立的电池放电模型基础上，重点检验模型对参数扰动的敏感性与稳健性，
以评估模型预测结果（电池可用时间）的可靠性。
按照解答文档的要求，第三问不引入新的控制或管理策略，
而是通过 **局部灵敏度分析** 与 **全局灵敏度分析**，
系统刻画关键参数及其交互效应。
...
- **等效电路模型 ECM（Q2）**
\[
V_{\mathrm{term}} = V_{\mathrm{oc}}(SOC,T)
- I R_0(SOH,T) - V_1 - V_2
\]
...
\[
\frac{dT}{dt}
= \frac{P_{\mathrm{heat}}}{C_{\mathrm{th}}}
- \frac{k_{\mathrm{cool}}}{C_{\mathrm{th}}}(T - T_{\mathrm{amb}})
\]

- **终止条件（Q2）**
\[
SOC \le SOC_{\min},\quad
V_{\mathrm{term}} \le V_{\mathrm{cutoff}},\quad
T \ge T_{\max}
\]

第三问中，所有仿真均使用与第二问一致的参数与阈值，仅对选定参数施加受控扰动，
以保证分析结果的可比性。
...
> 在参数存在不确定性或标定误差的情况下，
模型输出（电池可用时间 \(t_{\mathrm{empty}}\)）对各参数的敏感程度如何？
哪些参数是模型预测的关键决定因素？
是否存在显著的参数交互效应？

为此，按照解答文档的思路，本问采用两种互补的敏感性分析方法：
- **局部灵敏度分析（Tornado）**
- **全局灵敏度分析（Sobol 方差分解）**
...
在基准工况（选取第二问的典型场景 S1）附近，
对每一个参数 \(x_i\) 施加 ±10% 的小扰动，
计算电池可用时间 \(t_{\mathrm{empty}}\) 的变化，
并构造归一化弹性系数：

\[
S_i \approx
\frac{t_{\mathrm{empty}}(x_i(1+\delta))
- t_{\mathrm{empty}}(x_i(1-\delta))}
{2\delta\,t_{\mathrm{empty}}(x_0)}
\]

其中 \(\delta = 10\%\)，\(x_0\) 为基准参数值。
...
- 核心函数：`local_sensitivity_tornado()`
- 基准场景：`scenario_S1()`
- 被分析参数包括：
  - 等效容量 \(C_{\mathrm{eff}}\)
  - 内阻参考值 \(R_0\)
...
每次仅扰动一个参数，其余参数保持不变，
并重新运行完整的 Q2 模型仿真，得到新的 \(t_{\mathrm{empty}}\)。

---

### 3.3 输出结果与图像

- **输出图像**：`Q3-1_local_tornado.png`
- **图像含义**：
  - 横轴：局部灵敏度（弹性系数 \(S_i\)）
  - 纵轴：参数名称
  - 正负方向表示参数增加对续航时间的增减影响
  - 绝对值越大，说明该参数在基准点附近越关键
...
### 4.1 方法说明（对应文档“Sobol 全局分析”）

局部灵敏度分析仅反映基准点附近的线性响应，
无法揭示非线性行为和参数交互效应。

因此，第三问进一步采用 Sobol 方差分解方法，
将输出 \(Y=t_{\mathrm{empty}}\) 的总方差分解为：
...
其中 \(S_i\) 表示参数 \(x_i\) 的独立贡献，
而 \(S_{T_i}\) 则包含该参数与其他参数所有交互项的贡献。

---

### 4.2 代码实现说明

- 核心函数：`sobol_global_indices()`
- 采样方法：Sobol 序列（低差异采样）
- 样本规模：\(N = 256\)
- 总仿真次数约为 \((2 + d)N\)，其中 \(d\) 为参数维度
- 对每一个样本点：
  - 调用与 Q2 完全一致的仿真函数
  - 计算对应的 \(t_{\mathrm{empty}}\)

通过 Saltelli / Janon 型估计公式，
分别计算一阶与总阶 Sobol 指数。
...
- **输出图像**：`Q3-2_sobol_indices.png`
- **图像含义**：
  - 柱状对比：一阶 Sobol 指数 \(S_i\) 与总阶指数 \(S_{T_i}\)
  - 若 \(S_{T_i} \gg S_i\)：说明参数存在显著交互效应
  - 若二者接近：说明该参数影响近似独立、可加

该分析用于评估模型的非线性程度与参数耦合强度。
...
第三问还输出一张综合定量对比表，用于并列展示：

- 局部灵敏度结果（弹性系数与排名）
- Sobol 全局灵敏度结果（一阶 / 总阶）

该表仅保留定量结果，不包含管理或优化建议，
作为第四问的分析基础。
...
| Parameter | 参数名称 |
| Local_Si | 局部灵敏度（弹性系数） |
| LocalRank | 局部灵敏度重要性排名 |
| Sobol_Si | Sobol 一阶指数 |
| Sobol_STi | Sobol 总阶指数 |

该表可直接导入 Excel 或用于论文表格排版。

---

## 6. 第三问小结（不涉及第四问建议）

通过局部灵敏度与 Sobol 全局敏感性分析可以看出：

- 模型对少数物理本征参数（如容量、内阻、环境温度）高度敏感；
- 部分参数表现出显著的交互效应，说明模型存在非线性耦合；
- 对灵敏度较低的参数，将其视为常数是合理且稳健的简化假设。

上述结果验证了前两问模型的可信度，
并为后续第四问中的策略设计与优化分析提供了可靠依据。
