...
> 代码文件：`p2_submit.py`（或你当前的 `p2_other.py` 改进版）。
...
> 说明：代码里把占空比门控从“硬方波”换成“软方波”（tanh 平滑），目的是减少图像中的尖锐锯齿，让图更适合交稿阅读，但不会改变整体趋势和终止原因。
> 
> 
...
> 用法：你可以直接把 CSV 导入 Excel，做“表格 + 二次可视化”。
> 
> 
...
# 7. 备注（你当前版本的 event_timeline）

你说 `event_timeline.png` 都挂了，所以当前交稿版默认不生成。
如果你之后想要它，可以在脚本顶部把：

```python

MAKE_EVENT_TIMELINE = True
```

打开（此时已用“NaN→0 + 不用 tight 裁剪”修复，不会再生成细线图）。
